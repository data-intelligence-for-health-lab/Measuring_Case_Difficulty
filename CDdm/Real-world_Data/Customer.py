...
logging.info('Start of the app2_customer:' + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
customer_X  = customer_X.to_numpy()
customer_y = customer_y.to_numpy()
...
fold_index = []
for train_index, test_index in kfold.split(customer_X):
...
def approach_2(customer_X,customer_y,folds):
    ## First and Second folds
    # First fold
    X_fold = customer_X[fold_index[folds[0]]]
    y_fold = customer_y[fold_index[folds[0]]]
    d = pd.DataFrame(X_fold,columns=names)
    d['label'] = y_fold   
    fold_data0 = d
    
    # Second fold
    X_fold = customer_X[fold_index[folds[1]]]
    y_fold = customer_y[fold_index[folds[1]]]
    d = pd.DataFrame(X_fold,columns=names)
    d['label'] = y_fold   
    fold_data1 = d
    ...
    print("train_data_for_model_A\n",train_data_for_model_A['label'].value_counts())
    ...
    train_y_model_A = np_utils.to_categorical(train_y_model_A,4)  
    
    #StandardScaler be generated by fold 1 and fold 2
    train_x_model_A = preprocessor.fit_transform(train_x_model_A)
    ...
    # Third fold
    X_fold = customer_X[fold_index[folds[2]]]
    y_fold = customer_y[fold_index[folds[2]]]
    d = pd.DataFrame(X_fold,columns=names)
    d['label'] = y_fold   
    fold_data2 = d
    
    # Fourth fold
    X_fold = customer_X[fold_index[folds[3]]]
    y_fold = customer_y[fold_index[folds[3]]]
    d = pd.DataFrame(X_fold,columns=names)
    d['label'] = y_fold   
    fold_data3 = d
    ...
    print("test_data_for_model_A\n",test_data_for_model_A['label'].value_counts())
    ...
    model_A_X_test = preprocessor.transform(model_A_X_test)
    ...
    model_A_predictions = np.argmax(best_model_A.predict(np.array(model_A_X_test)), axis=1)
    ...
    model_A_X_test = preprocessor.fit_transform(model_A_X_test)
    ...
    X_fold = customer_X[fold_index[folds[4]]]
    y_fold = customer_y[fold_index[folds[4]]]
    d = pd.DataFrame(X_fold,columns=names)
    d['label'] = y_fold   
    difficulty_data_for_model_B = d
    print("difficulty_data_for_model_B\n",difficulty_data_for_model_B['label'].value_counts())   
    
    
    difficulty_x = difficulty_data_for_model_B.iloc[:,:-1]
    
    #Generated StandardScaler from fold 3 and fold 4 used for fold 5
    difficulty_x = preprocessor.transform(difficulty_x)      
...
for i in range(len(fold_order)):
    ...
    difficulty_data_for_model_B,difficulty = approach_2(customer_X,customer_y,folds)
...
new_column_names = names.tolist()
new_column_names.extend(['label','difficulty'])

temp_df= pd.DataFrame(np.row_stack([dataframe_temp[0], dataframe_temp[1], dataframe_temp[2], dataframe_temp[3], dataframe_temp[4]]), columns=new_column_names)
